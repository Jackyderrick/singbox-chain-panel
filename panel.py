...
<section><h2>运行状态</h2><div class="row"><button onclick="refresh()">刷新</button><button onclick="restart()">重启 sing-box</button><button onclick="loadLogs()">查看日志</button><button onclick="loadConnections()">刷新连接</button></div><p class="muted">服务器出口 IP：<span id="serverIp">...</span></p><p class="muted">默认家宽出口：<span id="activeHome">...</span></p></section>
...
<section class="full"><h2>当前连接流量</h2><p class="muted">上传：<span id="connUp">0 B</span> ｜ 下载：<span id="connDown">0 B</span></p><table><thead><tr><th>UUID</th><th>来源</th><th>目标</th><th>上传</th><th>下载</th><th>合计</th><th>链路</th></tr></thead><tbody id="connections"><tr><td colspan="7" class="muted">点击“刷新连接”加载</td></tr></tbody></table></section>
...
refresh().then(loadConnections).catch(showLogin)
setInterval(()=>{if(!document.getElementById('app').classList.contains('hidden'))loadConnections()},5000)
</script>
</body></html>"""
